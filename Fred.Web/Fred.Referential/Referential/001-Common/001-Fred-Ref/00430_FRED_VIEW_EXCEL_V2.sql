...
/****** Object:  View [dbo].[vw_FRED_Get_Commandes_For_Excel]    Script Date: 18/06/2018 11:30:14 ******/
IF OBJECT_ID('[dbo].[vw_FRED_Get_Commandes_For_Excel]', 'V') IS NOT NULL
	DROP VIEW [dbo].[vw_FRED_Get_Commandes_For_Excel]
GO
...
CREATE VIEW [dbo].[vw_FRED_Get_Commandes_For_Excel]
   AS  

	SELECT s.CodeSocieteComptable As Société, ec.code as Etab, ci.code+' - '+ci.libelle As CI, 
		co.[Date] As [Date Commande], 
		case when co.NumeroCommandeExterne is not null and co.NumeroCommandeExterne <> '' then co.NumeroCommandeExterne else co.Numero end as [Numéro Commande],
	...
	group by s.CodeSocieteComptable, ec.code, ci.code+' - '+ci.libelle, co.[Date], co.Numero, co.NumeroCommandeExterne,
