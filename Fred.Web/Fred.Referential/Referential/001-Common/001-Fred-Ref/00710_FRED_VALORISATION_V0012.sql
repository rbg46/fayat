...
INNER JOIN FRED_CONTRAT_INTERIMAIRE CCI on CCI.InterimaireId = P.PersonnelId AND CAST(RL.DatePointage AS DATE) BETWEEN CCI.DateDebut  AND DATEADD(DAY,cci.Souplesse,CCI.DateFin)
...
INNER JOIN FRED_SOCIETE_RESSOURCE_NATURE SRN ON SRN.RessourceId = CCI.RessourceId and SRN.SocieteId = s.SocieteId 
...
INSERT INTO FRED_VALORISATION (CiId, RapportId, RapportLigneId, TacheId, ChapitreId, SousChapitreId, ReferentielEtenduId, UniteId, DeviseId, PersonnelId, Date, VerrouPeriode, DateCreation, Source, PUHT, Quantite, Montant)
SELECT t.CiId, t.RapportId, T.RapportLigneId, T.TacheId,T.ChapitreId, T.SousChapitreId, T.ReferentielEtenduId, T.UniteId, T.DeviseId, T.PersonnelId as PersonnelId, T.[Date Pointage], 0 as VerrouPeriode,GETDATE() as DateCreation, 'Script Interim' as Source, T.PUHT, T.Quantite, T.Montant
FROM #TEMP t
