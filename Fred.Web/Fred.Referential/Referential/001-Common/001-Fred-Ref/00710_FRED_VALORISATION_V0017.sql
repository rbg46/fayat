-- TRAITEMENT PERSONNEL NON INTERIMAIRE POUR LA SOCIETE SOMOPA ET FAYAT TP
DECLARE
    @CiId int,
    @RapportId int,
    @RapportLigneId int,
    ...
    @Source nvarchar(max) = 'Script Personnel Non Interimaire',
    @PUHT decimal(18,3),
...
SELECT 
    ...
    rl.PersonnelId,
    rl.MaterielId,
    rl.DatePointage,res.SousChapitreId 
FROM 
    dbo.FRED_RAPPORT_LIGNE  rl WITH(NOLOCK)
    INNER JOIN FRED_PERSONNEL p WITH(NOLOCK) ON rl.PersonnelId = p.PersonnelId
    INNER JOIN FRED_SOCIETE_RESSOURCE_NATURE srn WITH(NOLOCK) ON 
        srn.RessourceId = p.RessourceId 
        AND srn.SocieteId = p.SocieteId
    INNER JOIN FRED_RESSOURCE res WITH(NOLOCK) ON res.RessourceId = srn.RessourceId
WHERE 
    rl.PersonnelId IS NOT NULL
    AND p.IsInterimaire = 0
    AND rl.CiId IN (
        SELECT ci.CiId 
        FROM 
            FRED_SOCIETE s WITH(NOLOCK)
            INNER JOIN FRED_CI ci WITH(NOLOCK) ON s.SocieteId = ci.SocieteId
        WHERE s.Code IN ('0001','0143')
    )
    AND (
        rl.RapportLigneId NOT IN (
            SELECT RapportLigneId 
            FROM FRED_VALORISATION v WITH(NOLOCK)
            WHERE 
                Quantite > 0 
                AND v.PersonnelId=rl.PersonnelId
        ) 
        OR rl.RapportLigneId IN (
            SELECT RapportLigneId 
            FROM FRED_VALORISATION WITH(NOLOCK) 
            WHERE Source LIKE 'Script%'
        )
    )
...
BEGIN
    ...
    SELECT TOP 1 @ChapitreId = ChapitreId 
    FROM FRED_SOUS_CHAPITRE WITH(NOLOCK) 
    WHERE SousChapitreId = @SousChapitreId;
    --Recuperer le TacheId
    INSERT INTO #RAPPORT_LIGNE_TACHE(TacheId, HeureTache)
    SELECT TacheId, HeureTache 
    FROM FRED_RAPPORT_LIGNE_TACHE WITH(NOLOCK) 
    WHERE 
        RapportLigneId = @RapportLigneId 
        AND HeureTache > 0;
    ...
    BEGIN
        ...
        BEGIN
            SELECT TOP 1  
                @Prix = Prix,
                @UniteId = UniteId,
                @PrixConduite = PrixConduite,
                @SurchargeReferentielEtenduId = ReferentielEtenduId
            FROM FRED_BAREME_EXPLOITATION_CI_SURCHARGE WITH(NOLOCK)
            WHERE 
                MaterielId = @MaterielId
                AND CiId = @CiId
        ...
        BEGIN
            SELECT 
                TOP 1   @Prix = Prix,@UniteId = UniteId, @PrixConduite = PrixConduite,@SurchargeReferentielEtenduId = ReferentielEtenduId
            FROM 
                FRED_BAREME_EXPLOITATION_CI_SURCHARGE WITH(NOLOCK)
            WHERE 
                PersonnelId = @PersonnelId 
                AND CiId = @CiId
                AND PeriodeDebut <= @DatePointage
        ...
        ELSE
        BEGIN
            SELECT TOP 1 @DeviseId = DeviseId 
            FROM FRED_DEVISE 
            WHERE IsoCode = 'EUR';
            SELECT TOP 1 @UniteId = UniteId  
            FROM FRED_UNITE WITH(NOLOCK) 
            WHERE Code = 'H';
            SET @PUHT = 0.01;
        ...
        BEGIN
            ...
            SELECT @SousChapitreId = SousChapitreId 
            FROM 
                FRED_SOCIETE_RESSOURCE_NATURE srn WITH(NOLOCK)
                INNER JOIN FRED_RESSOURCE res WITH(NOLOCK) ON res.RessourceId = srn.RessourceId
        ...
        BEGIN
            ...
            INSERT INTO [dbo].[FRED_VALORISATION](
                [CiId],
                [RapportId],
                [RapportLigneId],
                [TacheId],
