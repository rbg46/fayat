...
--Récupération des rapports sans valo à redresser (cas materiel)
	...
	AND (rl.RapportLigneId NOT IN (SELECT RapportLigneId FROM FRED_VALORISATION WITH(NOLOCK) WHERE Quantite > 0) OR
		...
		BEGIN
			...
			BEGIN
				...
				INSERT INTO [dbo].[FRED_VALORISATION]([CiId],[RapportId],[RapportLigneId],[TacheId],[ChapitreId],[SousChapitreId],[ReferentielEtenduId],[BaremeId],[BaremeStormId],
						[UniteId],[DeviseId],[PersonnelId],[MaterielId],[Date],[VerrouPeriode],[DateCreation],[Source],[PUHT],[Quantite],[Montant])
