...
--Récupération des rapports sans valo à redresser (cas personnel)
	...
	BEGIN
		...
		SELECT TOP 1 @RapportLigneId = RapportLigneId, @RapportId = RapportId, @PersonnelId = PersonnelId, @MaterielId = MaterielId, @CiId = CiId, @ReferentielEtenduId = ReferentielEtenduId, @DatePointage = DatePointage, @SousChapitreId = SousChapitreId,@IsInterimaire = IsInterimaire 
		FROM #RapportLignes
		...
		BEGIN
			...
			BEGIN
				...
				INSERT INTO [dbo].[FRED_VALORISATION]([CiId],[RapportId],[RapportLigneId],[TacheId],[ChapitreId],[SousChapitreId],[ReferentielEtenduId],[BaremeId],
						[BaremeStormId],[UniteId],[DeviseId],[PersonnelId],[Date],[VerrouPeriode],[DateCreation],[Source],[PUHT],[Quantite],[Montant])
