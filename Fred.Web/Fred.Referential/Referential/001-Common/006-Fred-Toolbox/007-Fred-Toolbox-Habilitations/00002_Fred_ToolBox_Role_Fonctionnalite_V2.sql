...
CREATE PROCEDURE Fred_ToolBox_Role_Fonctionnalite
        ...
        @FonctionnaliteCode  varchar(max),
...
DECLARE @PrimaryKeyValue INT;
		...
		SET @FonctionnaliteId = (SELECT f.FonctionnaliteId FROM FRED_FONCTIONNALITE f inner join FRED_MODULE m on f.ModuleId=m.ModuleId WHERE m.Code = @ModuleCode AND f.Code  = @FonctionnaliteCode)
