...
DECLARE @PrimaryKeyValue INT;
		...
		-- BEGIN FOREIGN KEYS
			...
			SET @RessourceID = 
							...
							WHERE FRED_SOUS_CHAPITRE.SousChapitreId IN 
								(
									...
									WHERE FRED_CHAPITRE.ChapitreId IN 
										(
										SELECT ChapitreId FROM FRED_CHAPITRE WHERE GroupeId = (SELECT GroupeId FROM FRED_GROUPE WHERE Code = @GroupeCode
										)
							...
							AND FRED_RESSOURCE.Code = @RessourceCode
							)
			...
			SET @ReferentielEtenduId = ( SELECT ReferentielEtenduId FROM FRED_SOCIETE_RESSOURCE_NATURE where RessourceId = @RessourceID AND NatureId = @NatureID AND SocieteId=@SocieteId)
