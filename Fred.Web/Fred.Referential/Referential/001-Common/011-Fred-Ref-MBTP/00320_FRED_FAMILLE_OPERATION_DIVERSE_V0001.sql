BEGIN 
	DECLARE @TacheMO			int, @TacheAchat		int, @TacheMateriel				int,  @TacheImmo		 int, @TacheAutre int, @SocieteId int
	...
	IF(@TacheMO IS NOT NULL AND @RessourceMo IS NOT NULL AND @SocieteId IS NOT NULL)
	BEGIN
	...
	IF(@TacheAchat IS NOT NULL AND @RessourceAchat IS NOT NULL AND @SocieteId IS NOT NULL) 
	BEGIN
	...
	IF(@TacheMateriel IS NOT NULL AND @RessourceMateriel IS NOT NULL AND @SocieteId IS NOT NULL)
	BEGIN
	...
	IF(@TacheImmo IS NOT NULL AND @RessourceAchat IS NOT NULL AND @SocieteId IS NOT NULL)
	BEGIN
		...
		BEGIN 
			Insert Into FRED_FAMILLE_OPERATION_DIVERSE ([Code],[Libelle],[SocieteId],[DateCreation],[AuteurCreationId],[DateModification],[AuteurModificationId],[IsAccrued],[MustHaveOrder],[IsValued],[TacheId],[RessourceId],[CategoryValorisationId])
			Values (N'MI',N'MATERIEL IMMOBILISE',@SocieteId,Getdate(),1,NULL,NULL,0,0,1,@TacheImmo,@RessourceAchat,NULL)
		END
		ELSE
		BEGIN
	...
	IF(@TacheAutre IS NOT NULL AND @RessourceAutre IS NOT NULL AND @SocieteId IS NOT NULL)
	BEGIN
		...
		BEGIN 
			Insert Into FRED_FAMILLE_OPERATION_DIVERSE ([Code],[Libelle],[SocieteId],[DateCreation],[AuteurCreationId],[DateModification],[AuteurModificationId],[IsAccrued],[MustHaveOrder],[IsValued],[TacheId],[RessourceId],[CategoryValorisationId])
			Values	(N'OTH',N'AUTRE DEPENSES SANS COMMANDE',@SocieteId,Getdate(),1,NULL,NULL,0,0,0,@TacheAutre,@RessourceAutre,NULL)
		END
		ELSE
		BEGIN
    ...
    BEGIN
	    UPDATE FRED_FAMILLE_OPERATION_DIVERSE SET TACHEID = (SELECT TOP 1 TACHEID FROM  FRED_TACHE where Libelle = 'Ecart Matériel Immobilisé' AND NIVEAU = 3  AND ParentId IS NOT NULL)
        WHERE 1=1 AND Code = 'MI' AND SocieteId = @SocieteId
    END
END