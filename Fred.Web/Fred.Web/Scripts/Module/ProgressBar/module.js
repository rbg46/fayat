...
angular.module('ngProgress.provider', ['ngProgress.directive'])
    .service('ngProgress', function () {
        ...
        return ['$document', '$window', '$compile', '$rootScope', '$timeout', function ($document, $window, $compile, $rootScope, $timeout) {
            ...
            this.blockScreenExceptMenu = false;

            // Starts the animation and adds between 0 - 5 percent to loading
            // each 400 milliseconds. Should always be finished with progressbar.complete()
            // to hide it
            // - blockScreenExceptMenu : indique si l'écran, sauf les menus, doit être bloqué durant la progression. Peut-être undefined.
            this.start = function (blockScreenExceptMenu) {
                this.blockScreenExceptMenu = blockScreenExceptMenu;
                if (blockScreenExceptMenu) {
                    $('#BLOCK_SCREEN_EXCEPT_MENU').addClass('block-screen-except-menu');
                }
            ...
            this.stop = function () {
                clearInterval(this.intervalCounterId);
                if (this.blockScreenExceptMenu) {
                    $('#BLOCK_SCREEN_EXCEPT_MENU').removeClass('block-screen-except-menu');
                }
