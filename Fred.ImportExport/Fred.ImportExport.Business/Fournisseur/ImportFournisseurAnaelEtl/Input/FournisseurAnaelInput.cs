...
namespace Fred.ImportExport.Business.Fournisseur.Etl.Input
{
    ...
    /// </summary>
    public class FournisseurAnaelInput : IEtlInput<FournisseurAnaelModel>
    {
        ...
        private string GetQuery()
        {
            ...
            string societes = "'" + CodeSocieteComptables + "','" + Flux.SocieteModeleCode + "'";

            // Test d'existence d'une date de dernière exécution
            if (!ByPassDate && Flux.DateDerniereExecution.HasValue)
            {
                byPassFiltreDate = 0;
                dateComparaison = Flux.DateDerniereExecution.Value.ToLocalTime().ToString("yyyyMMddHHmm");
            }

            return string.Format(sql, societes, TypeSequences, RegleGestions, byPassFiltreDate, dateComparaison);
        }
    }
}
