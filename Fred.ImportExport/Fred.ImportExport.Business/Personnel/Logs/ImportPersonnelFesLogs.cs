...
{
    public class ImportPersonnelFesLogs : BaseFredIELogger, IImportPersonnelLogs
    {
        public ImportPersonnelFesLogs()
          : base("IMPORT", "PERSONNEL_FES")
        {
        }
        ...
        public void LogStartRequestAllData()
        {
            NotThrowErrorInLog(() =>
          {
              Log("LOG-PFES-001]", "[RECUPERATION DES DONNEES NECESSAIRES A L IMPORT]");
          });
        }
        ...
        public void LogStartTransormPersonnel()
        {
            NotThrowErrorInLog(() =>
           {
               Log("LOG-PFES-002]", "[DEMARRAGE DE LA TRANSFORMATION ANAEL VERS FRED]");
           });
        }
        ...
        public void LogDetectionModificationOfPersonnel(List<PersonnelAffectationResult> personnelAffectationResults)
        {
            NotThrowErrorInLog(() =>
            {
                ...
                Log("LOG-PFES-003]", $"[DETECTION MODIFICATION DU PERSONNEL] Nombre de personnels modifiés {nb.Count}");
            });
        }
        ...
        public void LogNoModificationOfPersonnel(List<PersonnelAffectationResult> personnelAffectationResults)
        {
            NotThrowErrorInLog(() =>
            {
                ...
                Log("LOG-PFES-004]", $"[PAS DE MODIFICATION DU PERSONNELS] Nombre de personnels non modifiés {nb.Count}.");
            });
        }
            ...
            NotThrowErrorInLog(() =>
             {
                 ...
                 Log("[LOG-PFES-005]", $"[DETECTION CREATION PRESONNELS] Nombre de personnels non crées {nb.Count}.");
             });
        }
        ...
        public void WarnManagerNotFound(List<PersonnelAffectationResult> personnelAffectationResults)
        {
            NotThrowErrorInLog(() =>
            {
                ...
                {
                    Warn("[WARN-PFES-001]", warmMessage.ToString());
                }
            });
        }
        ...
        public void WarnEtablissementPaieNotFound(List<PersonnelAffectationResult> personnelAffectationResults)
        {
            NotThrowErrorInLog(() =>
            {
                ...
                if (!string.IsNullOrEmpty(warmMessage.ToString()))
                {
                    Warn("[WARN-PFES-002]", warmMessage.ToString());
                }
            });
        }
        ...
        public void WarnRessourceNotFound(List<PersonnelAffectationResult> personnelAffectationResults)
        {
            NotThrowErrorInLog(() =>
            {
                ...
                if (!string.IsNullOrEmpty(warmMessage.ToString()))
                {
                    Warn("[WARN-PFES-003]", warmMessage.ToString());
                }
            });
        }
