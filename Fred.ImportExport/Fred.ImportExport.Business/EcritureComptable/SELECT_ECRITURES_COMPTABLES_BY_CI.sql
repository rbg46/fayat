...
FROM AXSPE.SECAFRP
WHERE RSTE='{0}' AND TRIM(RSECT) ='{3}'  AND  CAST(SUBSTRING (DTCPTA, 7,4) || '-' || SUBSTRING (DTCPTA, 4,2) || '-' || SUBSTRING (DTCPTA, 1,2)  As DATE) >=  CAST('{1}' AS DATE) AND  CAST(SUBSTRING (DTCPTA, 7,4) || '-' ||SUBSTRING (DTCPTA, 4,2) || '-' || SUBSTRING (DTCPTA, 1,2)  As DATE)    <=  CAST('{2}' AS DATE) 
