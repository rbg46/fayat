...
{
    public class FonQueryExecutor : IFonQueryExecutor
    {
        ...
        public FonQueryExecutor(IControlePointageManager controlePointageManager, IRemonteeVracManager remonteeVracManager, IValidationPointageFonLogger logger)
        {
        ...
        public void ExecuteAnaelInserts<T>(ValidationPointageContextData globalData, IEnumerable<QueryInfo> pointagesAndPrimesQueries, T instance) where T : class
        {
            ...
            try
            {
                ...
                {
                    SendInsertQueries(destinationDatabase, FonQueryBuilder.InsertPointageClause, pointagesAndPrimesQueries.Where(q => q.IsPointage));
                    SendInsertQueries(destinationDatabase, FonQueryBuilder.InsertPrimeClause, pointagesAndPrimesQueries.Where(q => !q.IsPointage));
                }
            }
            catch (Exception e)
            {
                string errorMsg = this.logger.ErrorInInsertPointagesAndPrimes(e, currentQuery);
