...
namespace Fred.ImportExport.Business.ValidationPointage.Ftp.Mail
{
    internal static class ValidationPointageMailSenderFtp
    {
        private static readonly Dictionary<string, string> Recipients = new Dictionary<string, string>
        {
                { "Aziz AZIZI", "a.azizi@fayatit.fayat.com" },
                { "Christophe PETIT", "c.petit@fayatit.fayat.com" },
                { "Nicolas CALLOIX", "n.calloix@fayatit.fayat.com" },
                { "Cedric LEJORT", "c.lejort@fayatit.fayat.com" }
        };
        ...
        /// <param name="ftpQueryBuilder">Récupération des données</param>
        /// <param name="procQuery">Requête procédure AS400</param>
        public static void Send(ValidationPointageContextData globalData, IEnumerable<RapportLigneEnt> fredPointages, List<RapportPrimeLignePrimeEnt> listPrimeMensuelle, RvgPointagesAndPrimes rvgPointagesAndPrimes, DateTime periode, IFtpQueryBuilder ftpQueryBuilder, string procQuery)
        {
            ...
            body.AppendPointages(pointages, globalData.SocieteId);
            body.Append("<br /><br />");
            ...
            (IEnumerable<string> pointageQueries, IEnumerable<string> primeQueries) = ftpQueryBuilder.GetPointageAndPrimeQueries(globalData, periode, fredPointages, rvgPointagesAndPrimes);
